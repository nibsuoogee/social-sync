...
export const filterUpcomingEvents = (events, daysAhead) => {
    const now = new Date();
...
export const addOrUpdateEvent = async (event, calendarId, userId) => {
    const existingEvent = await EventDTO.findByUidAndCalendar(event.uid, calendarId);
    if (existingEvent) {
        const dbEvent = existingEvent;
        const hasChanged = dbEvent.title !== event.summary ||
            ...
            (dbEvent.timezone || "UTC") !== (event.timezone || "UTC") ||
            dbEvent.all_day !== !!(event.datetype === "date") ||
            (dbEvent.recurrence_rule || "") !== (event.rrule?.toString() || "");
        if (hasChanged) {
            const [updatedEvent, errUpdate] = await tryCatch(EventDTO.updateEvent(dbEvent.id, {
                title: event.summary,
        ...
        const [newEvent, errNewEvent] = await tryCatch(EventDTO.createEvent({
            ics_uid: event.uid,
...
export const createIcsFile = (events, calendar) => {
    const fmt = (d) => d ? new Date(d).toISOString().replace(/[-:]/g, "").split(".")[0] : "";
    const lines = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        "CALSCALE:GREGORIAN",
    ...
    events.forEach((event) => {
        lines.push("BEGIN:VEVENT", `UID:${event.ics_uid}`, `SUMMARY:${event.title || ""}`, `DESCRIPTION:${event.description || ""}`, `LOCATION:${event.location || ""}`, `DTSTART:${fmt(event.start_time)}`, `DTEND:${fmt(event.end_time)}`, `STATUS:${event.status || ""}`, "END:VEVENT");
    });
