...
import { calendarCreateBody, CalendarDTO, calendarModel, } from "../models/calendarModel";
...
export const calendarUrlImportRouter = new Elysia()
...
}, (app) => app.post("/import", async ({ body, user, error }) => {
    ...
    const [existingCalendar, errExistingCalendar] = await tryCatch(CalendarDTO.findByOwnerAndUrl(user.id, body.external_source_url));
    ...
    const [membership, errMembership] = await tryCatch(MembershipDTO.createMembership({
    ...
    if (errMembership)
        throw new Error(errMembership.message);
    if (!membership)
