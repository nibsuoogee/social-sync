...
export const exportCalendarRouter = new Elysia()
  ...
  .guard(
    ...
    (app) =>
      app.get("/calendar/export/:id", async ({ params, user, error }) => {
        ...
        const [hasMembership, errMembership] = await tryCatch(
          MembershipDTO.hasMembership(calendarId, user.id)
        );
        if (errMembership) return error(500, errMembership.message);
        if (!hasMembership) return error(401, "Unauthorized access to calendar");
        ...
        if (!calendar) return error(404, "Calendar not found");
